...
ccj_wrapper = function(graph_type, summarize_type=1, file_path="", locs=0){
    ...
    summary_fun <- switch(summarize_type, get_group_mean_vals, get_individual_mean_vals, get_group_mean_vals, get_individual_mean_vals)

    df <- summary_fun(df)

    return(
        graph_fun(df, var_names) +
        generate_facet(df, var_names)
    )
